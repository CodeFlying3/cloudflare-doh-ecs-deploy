...
import { envWithRules, makeQuery, makeResponseFromQuery } from './helpers';
...
describe('DoH worker', () => {
  ...
  it('falls back to CF-Connecting-IP when XFF has no public address', async () => {
    ...
    expect(response.status).toBe(200);
    expect(Array.from(forwarded.slice(start, start + 11))).toEqual([0, 8, 0, 7, 0, 1, 24, 0, 8, 8, 4]);
  });
  ...
  it('returns DNS SERVFAIL after exhausting the domestic group', async () => {
    ...
    expect(calls).toEqual(['https://dns.alidns.com/dns-query', 'https://doh.pub/dns-query']);
  ...
  it('falls back from Google to Cloudflare without crossing groups', async () => {
    ...
    expect(calls).toEqual(['https://dns.google/dns-query', 'https://cloudflare-dns.com/dns-query']);
  ...
  it('returns FORMERR for malformed DNS and empty HTTP errors for bad transport', async () => {
    ...
    const badGet = await handleRequest(new Request('https://worker.example/doh?dns=***'), env);
